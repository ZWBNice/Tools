...
#define PROGRESS_TIPLABEL_HEIGHT 24
...
- (void)configElementsOnAbove:(BOOL)flag {
    ...
    CGFloat unitLineWidth = (self.frame.size.width - 20 - 2 * PROGRESS_PADDING - 2 * pointRadius) / ([self.progressLevelArray count] - 1);
    
    for (NSInteger index = 0; index < [self.progressLevelArray count]; index++) {
        
        CGRect unitLineRect = CGRectMake(PROGRESS_PADDING+10 + index * unitLineWidth + pointRadius, (self.frame.size.height - lineHeight) / 2.0, unitLineWidth, lineHeight);
        CGRect pointRect = CGRectMake(PROGRESS_PADDING+10 + index * unitLineWidth, self.frame.size.height / 2.0 - pointRadius, 2 * pointRadius, 2 * pointRadius);
        ...
        CGRect timeRect;
        if (index%2 == 0) {
            if ([self.progressLevelArray count] == 6) {
                labelRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0+10), labelOriginY, unitLineWidth-15, PROGRESS_TIPLABEL_HEIGHT);
                timeRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0+10), labelOriginY-PROGRESS_TIPLABEL_HEIGHT, unitLineWidth, PROGRESS_TIPLABEL_HEIGHT);

            }else{
                labelRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0+10), labelOriginY, unitLineWidth-15, PROGRESS_TIPLABEL_HEIGHT);
                timeRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0+10), labelOriginY-PROGRESS_TIPLABEL_HEIGHT, unitLineWidth-15, PROGRESS_TIPLABEL_HEIGHT);
            }
            ...
            if ([self.progressLevelArray count] == 6) {
                ...
                // 适配订单详情timeline
                if (index == 5) {
                    labelRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0), labelOriginY+PROGRESS_TIPLABEL_HEIGHT+PROGRESS_TIPLABEL_HEIGHT, unitLineWidth, PROGRESS_TIPLABEL_HEIGHT);
                    timeRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0), labelOriginY+PROGRESS_TIPLABEL_HEIGHT+PROGRESS_TIPLABEL_HEIGHT+PROGRESS_TIPLABEL_HEIGHT, unitLineWidth, PROGRESS_TIPLABEL_HEIGHT);
                }else{
                    labelRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0+10), labelOriginY+PROGRESS_TIPLABEL_HEIGHT+PROGRESS_TIPLABEL_HEIGHT, unitLineWidth-15, PROGRESS_TIPLABEL_HEIGHT);
                    timeRect  = CGRectMake((pointRect.origin.x + pointRadius - unitLineWidth / 2.0+10), labelOriginY+PROGRESS_TIPLABEL_HEIGHT+PROGRESS_TIPLABEL_HEIGHT+PROGRESS_TIPLABEL_HEIGHT, unitLineWidth, PROGRESS_TIPLABEL_HEIGHT);
                }
        ...
        if (index <= self.currentLevel) {
            ...
            if (self.timeArray.count) {
                [self generateTimeLabelWithFrame:timeRect achieved:YES labelText:[self.timeArray objectAtIndex:index] above:flag];
            }
        } else {
            ...
            [self generateTipLabelWithFrame:labelRect
                                  ...
                                  labelText:[self.progressLevelArray objectAtIndex:index]
                                      above:flag];
            if (self.timeArray.count) {
                [self generateTimeLabelWithFrame:timeRect achieved:NO labelText:[self.timeArray objectAtIndex:index] above:flag];
            }
        }
    }
}
...
- (void)generateTimeLabelWithFrame:(CGRect)frame
                         achieved:(BOOL)achievedFlag
                        labelText:(NSString *)text
                            above:(BOOL)aboveFlag {
